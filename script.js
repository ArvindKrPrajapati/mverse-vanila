...
.then(data=> {
  ...
  data.data.map((item, index)=> {
    ...
    <b>${item.title}</b>
    <a href="/play/index.html?id=${item.tmdb_id}"  class="btn btn-danger">Watch Now</a>
    </div>
    </div>
    <img  src="${item.backdrop_path}" class="d-block w-100 c-img" alt="...">
    </div>`
    s += `<button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to=${index} ${index == 0 && 'class="active" aria-current="true"'} aria-label="Slide ${index+1}"></button>`
  })
